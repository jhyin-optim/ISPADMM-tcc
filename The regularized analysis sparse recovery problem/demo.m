...
data = [300 600;600 1200;800 1600;1000 2000;1500 3000;2000 4000;3000 6000;4000 8000];
...
for index = 1:row
    ...
    progress_r = [];
    for repeats=1:8
        p = 0.1*sqrt(n);%p = 100/n
        x0 = rand(n,1);%zeros(n,1)
        w0 = sprandn(n,1,p);
        A = randn(m, n);
        A = A*spdiags(1./sqrt(sum(A.^2))',0,n,n); %normlize columns
      ...
      for k = 1 : n
        ...
        u = M*w0; 
        d = A * u + sqrt(0.001)*randn(m,1);%

        disp('Starting ISPADMM_tcc')  
        [x, out1] = ISPADMM_tcc(x0, mu, opts);
        
        disp('Starting ISPADMM')
        [x, out2] = ISPADMM(x0, mu, opts);
