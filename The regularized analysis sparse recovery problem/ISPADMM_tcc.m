...
% （ADMM），利用 ADMM 求解 $L1$ 正则化稀疏恢复问题的原问题。
%
% 针对 $L1$正则化稀疏恢复问题
%
...
%
% 输入信息： $A$, $d$, $\mu$ ，迭代初始值 $x^0$ 以及提供各参数的结构体 |opts| 。
%
...
function [x, out] = ISPADMM_tcc(x0, mu, opts)
%%%
global A M d m n
% 迭代准备。
k = 0;
...
while k < opts.maxit && nrmC > opts.ftol
    ...
    %非精确求解 $x$ 子问题
    [outx,InItr,dn1] = CG(A,d,mu,x,w,y,baru,vn1,'ISPADMM_tcc',opts);
    x = outx;
    NinItr = NinItr + InItr;
    ...
    %求解 $w$ 子问题
    c =  eta2/se * barv + sigma/se * Mx + 1/se * bary;
    w = prox(c,1/se);
...
%%%
% $L1$正则化稀疏恢复问题的目标函数 $f(w)=\frac{1}{2 mu}\|Ax - d\|_2^2+ \|Mx\|_1$。
